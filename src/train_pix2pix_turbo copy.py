    ...
    for epoch in range(0, args.num_training_epochs):
        for step, batch in enumerate(dl_train):
            ...
            with accelerator.accumulate(*l_acc):
                ...
                #First forward pass without the adapters and collect the features
                
                # forward pass
                x_tgt_pred = net_pix2pix(x_src, prompt_tokens=batch["input_ids"], deterministic=True, gt_frame=None)
                ...
                loss_l1 = F.l1_loss(x_tgt_pred.float()*mask_src.repeat(1,3,1,1), x_tgt.float()*mask_src.repeat(1,3,1,1), reduction="mean") * args.lambda_l1
                loss_lpips = net_lpips(x_tgt_pred.float()*mask_src.repeat(1,3,1,1), x_tgt.float()*mask_src.repeat(1,3,1,1)).mean() * args.lambda_lpips
                ...
                if face_disc:
                    ...
                    lossD_real_face = net_disc_face(face_real.detach(), for_real=True).mean() * args.lambda_gan *2.0
                    accelerator.backward(lossD_real_face.mean())
                    if accelerator.sync_gradients:
                    ...
                    lossD_fake_face = net_disc_face(face_fake.detach(), for_real=False).mean() * args.lambda_gan *2.0
